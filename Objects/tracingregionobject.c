...
typedef enum {
    ...
    Py_MOVABLE_FREEZE = 2,
    // The reference is allowed. The object should be skipped
    Py_MOVABLE_COWN = 3,
} movable_status;
...
{
    if (_Py_IsDeepImmutable(obj) || Cown_Check(obj)) {
        return 0;
    }
...
mermaid_traverse(PyObject *obj, mermaid_dump_state_t *state)
{
    ...
    _PyObject_VisitReachable(obj, (visitproc)mermaid_visit, (void *)state);
}
...
static int _trace_visit(PyObject* obj, region_trace_state_t* state) {
    ...
    if (_PyImmutability_CanViewAsDeepImmutable(obj)) {
        assert(_Py_IsDeepImmutable(obj));
        return 0;
    }
