...
_PyModule_GetFilenameUTF8(PyObject *mod, char *buffer, Py_ssize_t maxlen)
{
    ...
    PyRegion_CLEARLOCAL(filenameobj);
    return size;
}
...
_Py_module_getattro(PyObject *self, PyObject *name)
{
    ...
    if (PyUnicode_Check(name) && PyUnicode_Compare(name, &_Py_ID(__dict__)) == 0) {
        return PyRegion_NewRef(m->md_dict);
    }
...
{
    m = _PyInterpreterState_GetModuleState(_PyObject_CAST(m));
    PyObject *dict = PyRegion_XNewRef(m->md_dict);
    if (dict == NULL) {
        return NULL;
    }
    if (!PyDict_Check(dict)) {
...
module_get_annotations(PyObject *self, void *Py_UNUSED(ignored))
{
    ...
    if (PyDict_GetItemRef(dict, &_Py_ID(__annotations__), &annotations) == 0) {
        ...
        PyRegion_RemoveLocalRef(annotate);
        Py_XDECREF(annotate);
