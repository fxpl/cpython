...
add_overwritten_fast_local(PyFrameObject *frame, PyObject *obj)
{
    ...
    for (Py_ssize_t i = 0; i < new_size - 1; i++) {
        ...
        if (PyRegion_AddLocalRef(o)) {
            PyRegion_RemoveLocalRef(obj);
            assert(!PyRegion_NeedsReadBarrier(new_tuple));
            Py_DECREF(new_tuple);
            return -1;
        }
    ...
    // Regions: This can be handled using a SETLOCAL since frames are unmoveable
...
framelocalsproxy_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    assert(!PyRegion_NeedsReadBarrier(frame));
...
/*[clinic end generated code: output=d6fe08335cf76ae4 input=e57380734815dac5]*/
{
    ...
    if (value != self->f_trace) {
        if (PyRegion_XSETLOCALNEWREF(self->f_trace, value)) {
            return -1;
        }
